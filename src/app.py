...
# ---------------------------------------------------------
# ESTILOS CSS PERSONALIZADOS (si quieres añadir)
# ---------------------------------------------------------
...
def crear_mapa_interactivo(instancia: InstanciaTSP, rutas: dict | None = None) -> folium.Map:
        ...
        for nombre_ruta, datos_ruta in rutas.items():
            if datos_ruta['ruta']:
                ...
                folium.PolyLine(
                    ...
                    popup=folium.Popup(
                        f"<b>{datos_ruta['nombre']}</b><br>Distancia: "
                        f"{datos_ruta['distancia']:.2f} km",
                        max_width=300
                    )
                ).add_to(mapa)
...
def crear_mapa_grafo_completo(instancia: InstanciaTSP) -> folium.Map:
    ...
    # Aristas entre todas las ciudades (líneas más visibles)
...
def main():
    ...
    st.markdown(
        '<h1 class="main-header">🚚 Optimizador de Rutas TSP - Chile</h1>',
        unsafe_allow_html=True
    )
    st.markdown("""
    <div style='text-align: center; margin-bottom: 2rem;'>
        <p style='font-size: 1.2rem; color: #666;'>
        Resuelve el Problema del Viajante (TSP) para ciudades de Chile usando diferentes algoritmos
        de optimización. Visualiza las rutas óptimas y compara métodos en tiempo real.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if instancia is None:
            st.error(
                "No se pudo cargar la instancia. Verifica que el archivo exista en la carpeta data/"
            )
            return
        ...
        with st.spinner('🧮 Calculando rutas optimizadas...'):
            ...
            if metodo in ["Exhaustivo", "Comparar ambos"]:
                try:
                    ...
                    # Método exhaustivo (L* y ruta*)
                    ruta_optima, dist_optima = resolver_exhaustivo(
                        instancia.matriz_distancias,
                        ciudad_inicio
                    )
            ...
            if metodo in ["Vecino Más Cercano", "Comparar ambos"]:
                try:
                    ...
                    ruta_nn, dist_nn = vecino_mas_cercano(
                        instancia.matriz_distancias,
                        ciudad_inicio
                    )
            ...
            gap = None
            if ruta_optima and ruta_nn and dist_optima and dist_optima > 0:
                # gap = ((L^NN - L*) / L*) * 100%
        ...
        with col_res1:
            if ruta_optima and dist_optima:
                mostrar_detalle_ruta(
                    ruta_optima,
                    dist_optima,
                    "Ruta Óptima - Método Exhaustivo"
                )

                with st.expander("📊 Ver tabla de aristas - Ruta Óptima"):
                    df_aristas_opt = construir_tabla_aristas(
                        instancia.matriz_distancias,
                        ruta_optima
                    )
                    st.dataframe(
                ...
                st.info(
                    "Selecciona 'Exhaustivo' o 'Comparar ambos' para ver la ruta óptima."
                )
        ...
        with col_res2:
            if ruta_nn and dist_nn:
                mostrar_detalle_ruta(
                    ruta_nn,
                    dist_nn,
                    "Ruta Heurística - Vecino Más Cercano"
                )

                with st.expander("📊 Ver tabla de aristas - Ruta Heurística"):
                    df_aristas_nn = construir_tabla_aristas(
                        instancia.matriz_distancias,
                        ruta_nn
                    )
                    st.dataframe(
                ...
                st.info(
                    "Selecciona 'Vecino Más Cercano' o 'Comparar ambos' para ver la ruta heurística."
                )
    ...
    with tab3:
        ...
        if (
            metodo == "Comparar ambos"
            and ruta_optima and ruta_nn
            and dist_optima and dist_nn
        ):
            col_graf1, col_graf2 = st.columns(2)

            with col_graf1:
                fig_dist = crear_grafico_comparacion(
                    dist_optima,
                    dist_nn,
                    tiempo_optimo,
                    tiempo_nn
            ...
            with col_graf2:
                fig_tiempo = crear_grafico_tiempos(
                    tiempo_optimo,
                    tiempo_nn
                )
            ...
            with col_eff1:
                if tiempo_nn and tiempo_nn > 0:
                    delta_txt = (
                        f"{eficiencia_relativa-1:.1f}x más rápido"
                        if eficiencia_relativa > 1
                        else f"{1-eficiencia_relativa:.1f}x más lento"
                    )
                    st.metric(
                        "Velocidad relativa",
                        f"{eficiencia_relativa:.1f}x",
                        delta=delta_txt
                    )
                else:
            ...
            with col_eff2:
                if dist_optima and dist_nn:
                    ...
                    delta_gap = f"{gap:.2f}%" if gap is not None else "N/A"
                    st.metric(
                        "Diferencia en distancia",
                        f"{diferencia_distancia:.2f} km",
                        delta=delta_gap
                    )

            with col_eff3:
                if dist_nn and dist_optima:
                    reduccion = (diferencia_distancia / dist_nn) * 100
                    st.metric(
                        "Reducción posible",
                        f"{reduccion:.2f}%",
            ...
            st.info(
                "Selecciona 'Comparar ambos' métodos para ver el análisis comparativo completo."
            )
    ...
    with tab4:
        ...
        with col_data1:
            ...
            st.dataframe(
                instancia.matriz_distancias.style
                .format("{:.2f}")
        ...
        with col_data2:
            ...
            distancias_values = distancias_values[distancias_values > 0]  # Excluir ceros
            ...
            if ruta_optima:
                info_text.append(
                    f"✅ **Ruta óptima encontrada** ({len(ruta_optima)} ciudades)"
                )
                info_text.append(
                    f"⏱️ Tiempo exhaustivo: {tiempo_optimo:.4f}s"
                )
            if ruta_nn:
                info_text.append(
                    f"✅ **Ruta heurística encontrada** ({len(ruta_nn)} ciudades)"
                )
                info_text.append(
                    f"⏱️ Tiempo heurístico: {tiempo_nn:.4f}s"
                )
            ...
            if info_text:
                ...
                st.info(
                    "No se han calculado rutas. Ajusta los parámetros en el sidebar."
                )
